...
task.spawn(function()
	while true and task.wait() do 
        ...
        for i,v in pairs(Cashiers:GetChildren()) do 
            if (math.round(v.Head.Position.X) == tonumber(-627)) then
                v:PivotTo(CFrame.new(-629.948, 24, -286.52))
            elseif (math.round(v.Head.Position.X) == tonumber(-624)) then
                v:PivotTo(CFrame.new(-622.948, 24, -286.52))
            end
        end
